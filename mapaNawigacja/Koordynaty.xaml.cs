        ...
        private async void szukajCelu(object sender, RoutedEventArgs e)
        {
            ...
            if (wynik.Status == MapLocationFinderStatus.Success)
            {
                try
                {
                    ...
                    DaneGeograficzne.odleglosc = (routeResult.Route.LengthInMeters / 1000).ToString();
                }
                catch (Exception)
                {
                    var dlg = new Windows.UI.Popups.MessageDialog(MapLocationFinderStatus.BadLocation.ToString(), DaneGeograficzne.opisCelu);
                    await dlg.ShowAsync();
